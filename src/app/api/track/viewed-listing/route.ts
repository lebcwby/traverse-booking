...
import { trackViewedListingServerSide } from "@/lib/server-tracking";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only Meta CAPI runs server-side; GA4 view_item is client-side only
    trackViewedListingServerSide(
      {
        listingId,
      ...
      console.error("[Server Tracking] viewed-listing error:", err)
    );
